...
module.exports = class PgPersistence {
  ...
  async deleteTodoList(todoListId) {
    let DELETE_TODO_LIST = "DELETE FROM todolists WHERE id = $1 AND username = $2";
  ...
  async markAllDone(todoListId) {
    const COPMLETE_ALL = "UPDATE todos SET done = TRUE" +
                          "  WHERE todoList_id = $1 AND NOT done AND username = $2";

    let result = await dbQuery(COPMLETE_ALL, todoListId, this.username);
    return result.rowCount > 0;
  }
  ...
  async existsTodoListTitle(title) {
    const CHECK_DUPLICATE = "SELECT * from todolists WHERE title = $1 AND username = $2";
    
    let result = await dbQuery(CHECK_DUPLICATE, title, this.username);
