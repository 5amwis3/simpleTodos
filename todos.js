...
app.post("/lists/:todoListId/todos/:todoId/destroy", 
  ...
  catchError(async (req, res) => {
  let { todoListId, todoId } = { ...req.params };
  let deleted = await res.locals.store.destroyTodo(+todoListId, +todoId);
...
app.post("/lists/:todoListId/todos",
  ...
  catchError(async (req, res) => {
    ...
    if (!errors.isEmpty()) {
      ...
      let created = await res.locals.store.addTodo(+todoListId, todoTitle);
...
app.post("/lists/:todoListId/edit",
  ...
  catchError(async (req, res) => {
    ...
    const rerenderEditList = catchError(async() => {
